...
async function runTests() {
    ...
    try {
        ...
        if (rooms.length > 0) {
            const room = rooms[0];
            const result = await client.sendTextMessage(room.roomId, 'Hello from SDK Test! ' + new Date().toISOString());
    ...
    try {
        ...
        if (rooms.length > 0) {
            ...
            console.log(`  Topic: ${room.currentState.getStateEvents('m.room.topic')[0]?.getContent().topic || '(未设置)'}`);
        } else {
            console.log('跳过: 没有房间');
        }
    } catch (err) {
