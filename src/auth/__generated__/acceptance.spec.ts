...
describe("assembly contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (AUTH_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            AUTH_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            AUTH_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = AUTH_ROUTES_ERROR_SCENARIOS;
        const errcodes = AUTH_ROUTES_ERRCODES;
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in assembly`);
            }
        }
    });
});
