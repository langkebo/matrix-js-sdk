 ...
 * Source: docs/api-contract/generated/modules/worker-admin.json
 */

import { describe, it, expect, vi, beforeEach } from "vitest";
import { WORKER_ADMIN_ROUTES } from "./route-table";
import { WORKER_ADMIN_ROUTES_STATUS_SCENARIOS, WORKER_ADMIN_ROUTES_ERROR_SCENARIOS, WORKER_ADMIN_ROUTES_ERRCODES } from "./contract-assertions";

describe("worker-admin contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (WORKER_ADMIN_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(s => s.status >= 200 && s.status < 300);
        if (!happyPath) return;

        // Verify that the route table contains at least one documented success route
        expect(WORKER_ADMIN_ROUTES.length).toBeGreaterThan(0);

        mockClient.http.authedRequest.mockResolvedValue({ success: true });
        const response = await mockClient.http.authedRequest({
            method: WORKER_ADMIN_ROUTES[0].method,
            path: WORKER_ADMIN_ROUTES[0].path,
        });
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const errorScenariosForFailure = WORKER_ADMIN_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = WORKER_ADMIN_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
        const errcodes = WORKER_ADMIN_ROUTES_ERRCODES as unknown as ReadonlyArray<{ errcode: string; httpStatus: string; note: string }>;
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in worker-admin`);
            }
        }
    });
});