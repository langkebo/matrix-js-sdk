...
import {
    MODULE_ROUTES_STATUS_SCENARIOS,
    MODULE_ROUTES_ERROR_SCENARIOS,
    MODULE_ROUTES_ERRCODES,
...
describe("module contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (MODULE_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            MODULE_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            MODULE_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = MODULE_ROUTES_ERROR_SCENARIOS;
        const errcodes = MODULE_ROUTES_ERRCODES;
