...
import {
    OPENCLAW_ROUTES_STATUS_SCENARIOS,
    OPENCLAW_ROUTES_ERROR_SCENARIOS,
    OPENCLAW_ROUTES_ERRCODES,
...
describe("openclaw contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (OPENCLAW_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            OPENCLAW_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            OPENCLAW_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = OPENCLAW_ROUTES_ERROR_SCENARIOS;
        const errcodes = OPENCLAW_ROUTES_ERRCODES;
