/*
 * AUTO-GENERATED by scripts/sdk-contract-codegen.mjs — DO NOT EDIT.
 * Regenerate via `pnpm run contract:codegen`.
 *
 * Module:        Key Rotation
 * Source:        docs/api-contract/generated/modules/key_rotation.json
 * Ledger schema: 1
 * Source profile: all
 * synapse-rust:  b3c5153fc5fa969a2caa04d8e506b18655b349a6
 */

/** Routes served by the synapse-rust `key_rotation` module. */
export const KEY_ROTATION_ROUTES = [
    { method: "GET", path: "/_matrix/client/v1/keys/rotation/check" },
    { method: "PUT", path: "/_matrix/client/v1/keys/rotation/config" },
    { method: "GET", path: "/_matrix/client/v1/keys/rotation/history/{device_id}" },
    { method: "POST", path: "/_matrix/client/v1/keys/rotation/revoke" },
    { method: "POST", path: "/_matrix/client/v1/keys/rotation/rotate" },
    { method: "GET", path: "/_matrix/client/v1/keys/rotation/status" },
] as const satisfies readonly { readonly method: string; readonly path: string }[];

/** Union of every (method, path) tuple in `KEY_ROTATION_ROUTES`. */
export type KeyRotationRoute = (typeof KEY_ROTATION_ROUTES)[number];

/** HTTP methods referenced by this module. */
export type KeyRotationMethod = KeyRotationRoute["method"];

/** Path templates referenced by this module. */
export type KeyRotationPath = KeyRotationRoute["path"];

/**
 * Recursively replace `{name}` placeholders with `${string}` so that
 * runtime template literals like ` `/friends/${userId}` ` satisfy this
 * type. Used by manager code that binds call sites to the ledger while
 * still interpolating path parameters.
 */
export type KeyRotationReplaceBraces<P extends string> =
    P extends `${infer A}{${infer ParamSegment}}${infer B}` ? ParamSegment extends string ? `${A}${string}${KeyRotationReplaceBraces<B>}` : never : P;

/** Broader path type that also accepts parametrised template literals. */
export type KeyRotationPathPattern = KeyRotationReplaceBraces<KeyRotationPath>;
