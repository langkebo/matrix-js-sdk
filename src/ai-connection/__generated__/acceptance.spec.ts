...
import {
    AI_CONNECTION_ROUTES_STATUS_SCENARIOS,
    AI_CONNECTION_ROUTES_ERROR_SCENARIOS,
    AI_CONNECTION_ROUTES_ERRCODES,
...
describe("ai_connection contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (AI_CONNECTION_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            AI_CONNECTION_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            AI_CONNECTION_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = AI_CONNECTION_ROUTES_ERROR_SCENARIOS;
        const errcodes = AI_CONNECTION_ROUTES_ERRCODES;
