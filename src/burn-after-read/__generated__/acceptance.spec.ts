...
import {
    BURN_AFTER_READ_ROUTES_STATUS_SCENARIOS,
    BURN_AFTER_READ_ROUTES_ERROR_SCENARIOS,
    BURN_AFTER_READ_ROUTES_ERRCODES,
...
describe("burn_after_read contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (
            BURN_AFTER_READ_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            BURN_AFTER_READ_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            BURN_AFTER_READ_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = BURN_AFTER_READ_ROUTES_ERROR_SCENARIOS;
        const errcodes = BURN_AFTER_READ_ROUTES_ERRCODES;
