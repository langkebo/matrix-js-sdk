...
describe("oidc contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (OIDC_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            OIDC_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            OIDC_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = OIDC_ROUTES_ERROR_SCENARIOS;
        const errcodes = OIDC_ROUTES_ERRCODES;
