 ...
 * Source: docs/api-contract/generated/modules/worker-body.json
 */
...
describe("worker-body contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        ...
        expect(WORKER_BODY_ROUTES.length).toBeGreaterThan(0);

        mockClient.http.authedRequest.mockResolvedValue({ success: true });
        const response = await mockClient.http.authedRequest({
            method: WORKER_BODY_ROUTES[0].method,
            path: WORKER_BODY_ROUTES[0].path,
        });
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in worker-body`);
            }
        }
    });
});