...
import { SYNC_ROUTES_STATUS_SCENARIOS, SYNC_ROUTES_ERROR_SCENARIOS, SYNC_ROUTES_ERRCODES } from "./contract-assertions";

describe("sync contract acceptance", () => {
    let mockClient: any;
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (SYNC_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(s => s.status >= 200 && s.status < 300);
        if (!happyPath) return;

        // Verify that the route table contains at least one documented success route
        expect(SYNC_ROUTES.length).toBeGreaterThan(0);

        mockClient.http.authedRequest.mockResolvedValue({ success: true });
        const response = await mockClient.http.authedRequest({
            method: SYNC_ROUTES[0].method,
            path: SYNC_ROUTES[0].path,
        });
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const errorScenariosForFailure = SYNC_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = SYNC_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
        const errcodes = SYNC_ROUTES_ERRCODES as unknown as ReadonlyArray<{ errcode: string; httpStatus: string; note: string }>;
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in sync`);
            }
        }
    });
});