...
import {
    EXTERNAL_SERVICE_ROUTES_STATUS_SCENARIOS,
    EXTERNAL_SERVICE_ROUTES_ERROR_SCENARIOS,
    EXTERNAL_SERVICE_ROUTES_ERRCODES,
...
describe("external_service contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (
            EXTERNAL_SERVICE_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>
        ).find((s) => s.status >= 200 && s.status < 300);
        if (!happyPath) return;
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const failureScenario =
            EXTERNAL_SERVICE_ROUTES_ERROR_SCENARIOS.find((s) => /\b(401|400)\b/.test(s.httpOrErrcode)) ??
            EXTERNAL_SERVICE_ROUTES_ERROR_SCENARIOS.find((s) => /\b4\d\d\b/.test(s.httpOrErrcode));
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = EXTERNAL_SERVICE_ROUTES_ERROR_SCENARIOS;
        const errcodes = EXTERNAL_SERVICE_ROUTES_ERRCODES;
