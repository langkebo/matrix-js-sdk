 ...
 * Source: docs/api-contract/generated/modules/notifications.json
 */

import { describe, it, expect, vi, beforeEach } from "vitest";
import { NOTIFICATIONS_ROUTES } from "./route-table";
import { NOTIFICATIONS_ROUTES_STATUS_SCENARIOS, NOTIFICATIONS_ROUTES_ERROR_SCENARIOS, NOTIFICATIONS_ROUTES_ERRCODES } from "./contract-assertions";

describe("notifications contract acceptance", () => {
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (NOTIFICATIONS_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(s => s.status >= 200 && s.status < 300);
        if (!happyPath) return;

        // Verify that the route table contains at least one documented success route
        expect(NOTIFICATIONS_ROUTES.length).toBeGreaterThan(0);

        mockClient.http.authedRequest.mockResolvedValue({ success: true });
        const response = await mockClient.http.authedRequest({
            method: NOTIFICATIONS_ROUTES[0].method,
            path: NOTIFICATIONS_ROUTES[0].path,
        });
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const errorScenariosForFailure = NOTIFICATIONS_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = NOTIFICATIONS_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
        const errcodes = NOTIFICATIONS_ROUTES_ERRCODES as unknown as ReadonlyArray<{ errcode: string; httpStatus: string; note: string }>;
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in notifications`);
            }
        }
    });
});