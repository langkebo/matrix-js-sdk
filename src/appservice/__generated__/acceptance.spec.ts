...
import { APP_SERVICE_ROUTES_STATUS_SCENARIOS, APP_SERVICE_ROUTES_ERROR_SCENARIOS, APP_SERVICE_ROUTES_ERRCODES } from "./contract-assertions";

describe("app_service contract acceptance", () => {
    let mockClient: any;
    ...
    it("should satisfy a documented 2xx happy-path status", async () => {
        const happyPath = (APP_SERVICE_ROUTES_STATUS_SCENARIOS as unknown as ReadonlyArray<{ status: number }>).find(s => s.status >= 200 && s.status < 300);
        if (!happyPath) return;

        // Verify that the route table contains at least one documented success route
        expect(APP_SERVICE_ROUTES.length).toBeGreaterThan(0);

        mockClient.http.authedRequest.mockResolvedValue({ success: true });
        const response = await mockClient.http.authedRequest({
            method: APP_SERVICE_ROUTES[0].method,
            path: APP_SERVICE_ROUTES[0].path,
        });
    ...
    it("should handle a documented 4xx failure branch", async () => {
        const errorScenariosForFailure = APP_SERVICE_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
    ...
    it("should correctly map typed-error branches and errcodes", async () => {
        const errorScenarios = APP_SERVICE_ROUTES_ERROR_SCENARIOS as unknown as ReadonlyArray<{ scenario: string; httpOrErrcode: string; sdkErrorType: string; handling: string }>;
        const errcodes = APP_SERVICE_ROUTES_ERRCODES as unknown as ReadonlyArray<{ errcode: string; httpStatus: string; note: string }>;
        ...
        for (const code of errcodes) {
            ...
            if (!hasHandling) {
                // console.warn(`No specific error scenario handling for errcode ${code.errcode} in app_service`);
            }
        }
    });
});