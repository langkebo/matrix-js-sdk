...
function runCommand(cmd, silent = false) {
    try {
        return execSync(cmd, { encoding: "utf8", cwd: projectRoot, stdio: silent ? "pipe" : "inherit" });
    } catch (e) {
        return null;
    }
}
...
function collectTestStats() {
    const result = runCommand("npx vitest run --reporter=json 2>/dev/null", true);
    if (!result) {
        return { error: "Test run failed" };
    }
...
function generateMarkdownSummary(report) {
    ...
    const lines = [
        ...
        `| Passed | ${report.metrics.tests?.passed || 0} |`,
        `| Failed | ${report.metrics.tests?.failed || 0} |`,
        `| Skipped | ${report.metrics.tests?.skipped || 0} |`,
        "",
        "---",
