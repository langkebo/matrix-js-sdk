...
import { MatrixRTCSession, MatrixRTCSessionEvent } from "../../../src/matrix-rtc/MatrixRTCSession";
import { MembershipManagerEvent } from "../../../src/matrix-rtc/IMembershipManager";
import { Status, type EncryptionKeysEventContent } from "../../../src/matrix-rtc/types";
import {
    makeMockEvent,
    makeMockRoom,
    membershipTemplate,
...
import { RTCEncryptionManager } from "../../../src/matrix-rtc/RTCEncryptionManager.ts";
import { RoomStickyEventsEvent, type StickyMatrixEvent } from "../../../src/models/room-sticky-events.ts";
import { StickyEventMembershipManager } from "../../../src/matrix-rtc/MembershipManager.ts";
import { type CallMembershipIdentityParts } from "../../../src/matrix-rtc/EncryptionManager.ts";
...
describe("MatrixRTCSession", () => {
    ...
    describe("roomsessionForSlot combined state", () => {
        ...
        it("keeps joined state membership when matching sticky event is left", async () => {
            const stickyUserId = "@stickyleft:user.example";
            const joinedMembership = {
                ...membershipTemplate,
                user_id: stickyUserId,
            };
            const mockRoom = makeMockRoom([joinedMembership]);
            mockRoom._unstable_getStickyEvents.mockImplementation(() => {
                const ev = mockRTCEvent(
                    {
                        user_id: stickyUserId,
