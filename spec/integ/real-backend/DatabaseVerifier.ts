...
export class DatabaseVerifier {
  ...
  private parseDataLine(line: string): string[] {
    ...
    const words = line.trim().split(/\s{2,}/).map(v => v.trim()).filter(Boolean);
    return words;
  }
  ...
  async getTableColumns(tableName: string): Promise<string[]> {
    ...
    return lines.map(line => {
      ...
      const trimmed = line.trim();
      if (trimmed.includes('|')) {
        return trimmed.split('|').map(v => v.trim()).filter(Boolean)[0] || '';
      }
  ...
  async verifyNoTimestampViolations(): Promise<{ passed: boolean; violations: Array<{ table: string; column: string }> }> {
