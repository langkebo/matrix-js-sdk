...
| `./dehydrated-device`  | Dehydrated Device 白名单：脱水设备 manager 与脱水设备类型       | `DehydratedDeviceManager`, `DehydratedDeviceData`, `DeviceInfo`               |
| `./key-rotation`       | Key Rotation 白名单：密钥轮换 manager 与轮换状态/历史类型       | `KeyRotationManager`, `KeyRotationStatus`, `KeyRotationHistory`               |
